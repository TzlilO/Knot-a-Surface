...
def add_save_ply_to_multi_surface_model(MultiSurfaceSplineModel):
    ...
    def save_ply(self, path: str, viewpoint_cam=None):
        ...
        for i, (surface, active) in enumerate(zip(self.surfaces, self._active_surfaces)):
            ...
            elif hasattr(surface, 'surface_normal'):
                normals_list.append(surface.surface_normal().reshape(-1, 3))
            else:
                normals_list.append(surface.get_smallest_axis().reshape(-1, 3))
            ...
            opacities_list.append(
                # surface.opacity.cache if surface.opacity.cache.view(-1, 1) is not None else
                surface.get_opacity
            )
