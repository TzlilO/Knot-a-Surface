...
class Camera(nn.Module):
    ...
    def world_to_camera(self, points):
        ...
        Returns: [N, 3] torch tensor (camera/view coordinates)
        """
        # Add a column of ones to get homogeneous coordinates
        ones = torch.ones((*points.shape[:-1], 1), dtype=points.dtype, device=points.device)
        points_h = torch.cat([points, ones], dim=-1)  # [N, 4]
        # Apply world_view_transform (should be [4, 4])
        cam_points_h = torch.matmul(points_h, self.world_view_transform)  # [N, 4]
        return cam_points_h[..., :3].reshape(-1, 3)


    @property
    def w2c(self):
        return torch.tensor(getWorld2View2(self.R, self.T).tolist(), device=self.data_device)
    ...
    # --- NEW METHOD (THE FIX) ---
    def project_points(self, cam_points):
        """
        A convenience wrapper for camera_to_image to match the loss function's API.
        """
        return self.camera_to_image(cam_points)

    # --- NEW METHOD (Needed for consistency losses) ---
    def unproject_points(self, pixel_coords, depth_values):
        """
        Unprojects 2D pixel coordinates with depth values to 3D camera space.

        Args:
            pixel_coords (torch.Tensor): Tensor of shape [N, 2] with (u, v) coordinates.
            depth_values (torch.Tensor): Tensor of shape [N] with depth values.

        Returns:
            torch.Tensor: Tensor of shape [N, 3] with (x, y, z) coordinates in camera space.
        """
        u, v = pixel_coords.unbind(-1)
        z = depth_values
    ...
    def camera_to_image(self, cam_points):
        """
        cam_points: [N, 3] torch tensor (camera/view coordinates)
        Returns: [N, 2] torch tensor (pixel coordinates, float, not integer)
        """
        x = cam_points[:, 0]
        y = cam_points[:, 1]
        z = cam_points[:, 2].clamp(min=1e-6)  # Avoid division by zero
        # Use intrinsics (Fx, Fy, Cx, Cy)
    ...
    def get_rays(self, scale=1.0, size=None):
        if size is not None:
            H, W = size
        else:
    ...
    def get_rays2(self, scale=1.0, subsample_rate=1.0, return_origins=False):
        W, H = int(self.image_width / scale), int(self.image_height / scale)
        if subsample_rate < 1.0:
            # Subsample grid (e.g., for faster analysis)
            ix = torch.arange(0, W, int(1 / subsample_rate))
            iy = torch.arange(0, H, int(1 / subsample_rate))
            ix, iy = torch.meshgrid(ix, iy, indexing='xy')
        else:
            ix, iy = torch.meshgrid(torch.arange(W), torch.arange(H), indexing='xy')
        rays_d = torch.stack(
            [(ix - self.Cx / scale) / self.Fx * scale,
             (iy - self.Cy / scale) / self.Fy * scale,
             torch.ones_like(ix)], -1).float().to(self.device)
        if return_origins:
            # Ray origins (camera center in world)
            c2w = self.world_view_transform.transpose(0, 1).inverse()
            ray_o = c2w[:3, 3].expand_as(rays_d[..., :3])
            return rays_d, ray_o
        return rays_d
