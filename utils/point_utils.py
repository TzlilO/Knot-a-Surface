...
def depth_to_pts(fov_camera, depth, scale=1):
    Us, Vs = depth.shape[1], depth.shape[2]
    st = int(max(int(scale/2)-1,0))
    depth_view = depth.squeeze()[st::scale,st::scale]
    rays_d = fov_camera.get_rays(size=(Us, Vs))
...
def depth_to_normal(view, depth):
    """
        ...
        depth: depthmap
    """
    points = depth_to_pts(view, depth).reshape(*depth.shape[1:], 3)
