...
def render(viewpoint_camera, nurbs: 'MultiSurfaceSplineModel', pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None,
    ...
    nurbs.forward(viewpoint_camera)
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = torch.zeros_like(nurbs.get_xyz, dtype=nurbs.get_xyz.dtype, requires_grad=True, device="cuda") + 0
    screenspace_points_abs = torch.zeros_like(nurbs.get_xyz, dtype=nurbs.get_xyz.dtype, requires_grad=True, device="cuda") + 0
    try:
    ...
    if pipe.compute_cov3D_python:
        cov3D_precomp = nurbs.get_covariance(scaling_modifier)
    else:
        scales = nurbs.get_scaling
        rotations = nurbs.get_rotation
    ...
    raster_settings = PlaneGaussianRasterizationSettings(
            ...
            sh_degree=nurbs.active_sh_degree,
    ...
    if not return_plane:
        ...
        if app_model is not None and nurbs.use_app:
    ...
    global_normal = nurbs.get_normal(viewpoint_camera)
    ...
    #     rendered_image = rendered_image * (rendered_alpha).detach()
